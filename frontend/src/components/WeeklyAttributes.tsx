...
const WeeklyAttributes: React.FC<WeeklyAttributesProps> = ({
  weekNumber,
  workbookId,
  onAttributesChange,
}) => {
  ...
  const refreshAttributes = useCallback(async () => {
    try {
      ...
      const selectedIds = selectedRes.data.map(
        (item: WeeklyGraduateAttribute) => item.graduate_attribute_id
      );
      ...
      try {
        ...
        if (graduateAttributes.length === 0) {
          const attributesRes = await axios.get(
            `${process.env.VITE_API}/graduate_attributes/`
          );
          if (!mounted) return;
          ...
          attributes.forEach((attr: GraduateAttribute) => {
            ...
            if (!graduateAttributeColors[normalizedKey]) {
              console.warn(
                `No color mapping found for attribute: ${attr.name} (key: ${normalizedKey})`
              );
            }
          });
        ...
        if (!mounted) return;
        setError(`Failed to fetch graduate attributes. Error: ${getErrorMessage(error)}`);
          ...
          {ifCourseLead ? (
            ...
            <>
              {selectedAttributes[0] ? (
                <CustomBadge
                  label={selectedAttributes[0].name}
                  colorMapping={graduateAttributeColors}
                />
              ) : null}
              {selectedAttributes[1] ? (
                <CustomBadge
                  label={selectedAttributes[1].name}
                  colorMapping={graduateAttributeColors}
                />
              ) : null}
            </>
...
const DropdownRadio: React.FC<DropdownRadioProps> = ({
  ...
  return (
    <div className="relative inline-block w-full" ref={dropdownRef}>
      <button
        ...
        style={{
          backgroundColor: selected
            ? graduateAttributeColors[normalizeKey(selected.name)]
            : '#6c757d',
        }}
      ...
      >
        {selected ? (
          <CustomBadge label={selected.name} colorMapping={graduateAttributeColors} />
        ) : (
              ...
              <li key={attr.id}>
                <div
                  ...
                  style={{
                    backgroundColor:
                      selected?.id === attr.id
                        ? graduateAttributeColors[normalizeKey(attr.name)]
                        : 'transparent',
                  }}
                  onClick={() => {
