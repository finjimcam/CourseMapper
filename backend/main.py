...
# Import database-related methods and models
from backend.models.database import create_db_and_tables, get_session, populate_initial_data
from backend.models.models import (
    User, PermissionsGroup, Course, LearningPlatform, LearningActivity, TaskStatus, LearningType
)
...
def read_permissions_groups(session: Session = Depends(get_session)):
...
def read_learning_platforms(session: Session = Depends(get_session)):
...
def read_learning_activities(session: Session = Depends(get_session)):
...
def read_task_statuses(session: Session = Depends(get_session)):
    task_statuses = session.exec(select(TaskStatus)).all()
    return task_statuses

# Endpoint to fetch all LearningTypes
@app.get("/learning-types/")
def read_learning_types(session: Session = Depends(get_session)):
